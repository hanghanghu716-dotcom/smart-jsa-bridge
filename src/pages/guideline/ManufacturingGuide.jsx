...
/**
 * [ManufacturingGuide 컴포넌트]
 * 역할: 제조·생산 공정 JSA 견본 가이드 제공 (글로벌 표준 마이그레이션 완료)
 */
...
export default function ManufacturingGuide() {
  ...
  const { t } = useTranslation('manu'); // manu 네임스페이스 사용
  const [isMenuOpen, setIsMenuOpen] = useState(false);

  // ✅ [수정] 하드코딩된 리스트를 다국어 리소스에서 호출
  const manufacturingJsaList = t('list', { returnObjects: true }) || [];
        ...
        <div style={styles.centerContent}>
          ...
          <section style={styles.section} className="max-lg:!py-16 max-lg:!px-6">
            ...
            <div style={styles.listContainer}>
              {Array.isArray(manufacturingJsaList) && manufacturingJsaList.map((item) => (
                <div key={item.id} style={styles.card} className="max-lg:!p-6 max-lg:!rounded-2xl">
                  ...
                  <div style={styles.imageContainer}>
                    ...
                    {Array.from({ length: item.pages || 1 }, (_, i) => (
                      <img 
                        key={i}
                        src={`/assets/pdf/manu/${item.id}/${item.id}_00${i + 1}.jpg`}
                  ...
                  <div style={styles.cardFooter}>
                    <a href={`/assets/pdf/manu/${item.id}.pdf`} download style={styles.downloadBtn} className="max-lg:!w-full max-lg:!text-sm">
                      {t('ui.downloadBtn')}
                    </a>
                  </div>
                </div>
...
const styles = {
  ...
  categoryTag: { color: '#2e7d32', fontWeight: '900', fontSize: '0.8rem', letterSpacing: '2.5px', display: 'block', marginBottom: '20px' },
  ...
  jsaId: { color: '#2e7d32', fontWeight: '900', fontSize: '0.85rem' },
  ...
  infoBox: { padding: '20px', borderRadius: '12px', backgroundColor: '#f9fbf9', border: '1px solid #edf2ed' },
  labelRed: { color: '#d32f2f', fontSize: '0.8rem', display: 'block', marginBottom: '8px' },
  labelBlue: { color: '#2e7d32', fontSize: '0.8rem', display: 'block', marginBottom: '8px' },
