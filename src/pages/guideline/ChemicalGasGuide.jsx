...
/**
 * [ChemicalGasGuide 컴포넌트]
 * 역할: 화공·가스 작업 JSA 견본 가이드 제공 (글로벌 표준 마이그레이션 완료)
 */

// 법적 고지 컴포넌트: 다국어 적용 및 전달받은 t 사용[cite: 19]
...
export default function ChemicalGasGuide() {
  const navigate = useLanguageNavigate(); // ✅ [변경] 커스텀 다국어 네비게이트 사용[cite: 11]
  const { t } = useTranslation('chem'); // chem 네임스페이스 사용 (추후 json 등록 필요)[cite: 18]
  const [isMenuOpen, setIsMenuOpen] = useState(false);

  // ✅ [수정] 하드코딩된 리스트를 다국어 리소스에서 호출[cite: 17, 19]
  const chemicalJsaList = t('list', { returnObjects: true }) || [];

  return (
    <div style={styles.wrapper}>
      <SEO /> {/* ✅ [추가] 글로벌 SEO 태그 자동 삽입 */}

      {/* HEADER: 원본 스타일 및 로직 유지 */}
        ...
        <div style={styles.centerContent}>
          ...
          <section style={styles.section} className="max-lg:!py-16 max-lg:!px-6">
            ...
            <div style={styles.listContainer}>
              {Array.isArray(chemicalJsaList) && chemicalJsaList.map((item) => (
...
// 스타일 객체는 원본 데이터를 절대적으로 유지합니다.[cite: 16]
const styles = {
