...
/**
 * [HighRiskGuide 컴포넌트]
 * 역할: 고위험 특수작업 JSA 견본 가이드 제공 (글로벌 표준 마이그레이션 완료)
 */
...
export default function HighRiskGuide() {
  ...
  const { t } = useTranslation('highrisk'); // highrisk 네임스페이스 사용
  const [isMenuOpen, setIsMenuOpen] = useState(false);

  // ✅ [수정] 하드코딩된 리스트를 다국어 리소스에서 호출
  const highRiskList = t('list', { returnObjects: true }) || [];
        ...
        <div style={styles.centerContent}>
          <section style={styles.section} className="max-lg:!py-16 max-lg:!px-6">
            ...
            <div style={styles.listContainer}>
              {Array.isArray(highRiskList) && highRiskList.map((item) => (
...
// 스타일 객체는 원본 소스코드를 100% 보존합니다[cite: 19].
const styles = {
