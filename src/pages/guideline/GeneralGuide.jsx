...
/**
 * [GeneralGuide 컴포넌트]
 * 역할: 기타·일반 작업 JSA 견본 가이드 제공 (글로벌 표준 마이그레이션 완료)
 */
...
export default function GeneralGuide() {
  ...
  const { t } = useTranslation('general'); // general 네임스페이스 사용
  const [isMenuOpen, setIsMenuOpen] = useState(false);

  // ✅ [수정] 하드코딩된 리스트를 다국어 리소스에서 호출
  const generalJsaList = t('list', { returnObjects: true }) || [];
        ...
        <div style={styles.centerContent}>
          <section style={styles.section} className="max-lg:!py-16 max-lg:!px-6">
            ...
            <div style={styles.listContainer}>
              {Array.isArray(generalJsaList) && generalJsaList.map((item) => (
